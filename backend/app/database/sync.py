...
Pulls all records from key objects and upserts into local PostgreSQL tables.
"""
import logging
import asyncio
import threading
...
async def _sync_students(session):
...
async def _sync_submissions(session):
    logger.info("Syncing Submissions__c...")
    records = await _fetch_all(
...
async def _sync_interviews(session):
    logger.info("Syncing Interviews__c...")
    records = await _fetch_all(
...
async def _sync_managers(session):
    logger.info("Syncing Manager__c...")
    records = await _fetch_all(
...
async def _sync_jobs(session):
    logger.info("Syncing Job__c...")
    records = await _fetch_all(
...
async def _sync_employees(session):
